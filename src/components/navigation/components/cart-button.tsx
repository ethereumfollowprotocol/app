...
const CartButton = ({ cartItemsCount }: { cartItemsCount: number }) => {
  ...
  return (
    <Link href='/editor' passHref={true} legacyBehavior={true}>
      <div
        className={clsx(
          'glass-card border-2 h-[48px] justify-center items-center opacity-40 w-[48px] border-darkGrey transition-opacity cursor-pointer hover:opacity-80 relative flex rounded-full',
          pathname === '/editor' && 'opacity-100'
        )}
      >
        <Image src={Cart} alt='cart button' width='22' />
