...
const TopEightProfile: React.FC<TopEightProfileProps> = ({ profile, isEditing }) => {
  ...
  const isAddingToTopEight =
    isEditing && hasListOpAddTag({ address: profile.address, tag: "top8" });
  ...
  return (
    <div
      className={cn(
        ...
        isEditing
          ...
          : "w-[129px] 2xl:w-36 hover:bg-text/5 h-[177px]",
    ...
    >
      ...
      {followerTag && (
        <div
          className={cn(
            "rounded-full absolute font-bold text-[10px] flex bottom-[60px] items-center justify-center bg-zinc-300 h-5 min-w-20 w-fit px-1",
