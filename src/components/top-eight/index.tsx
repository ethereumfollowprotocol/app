...
const TopEight: React.FC<TopEightProps> = ({ user, isConnectedUserProfile }) => {
  ...
  return (
    <>
      ...
      <div className='glass-card relative xl:w-76 w-full 2xl:w-86 border-[3px] p-6 rounded-xl flex flex-col gap-6 border-[#FFDBD9] dark:border-[#a36d7d]'>
        {isConnectedUserProfile && (
          <div
            onClick={() => setEditModalOpen(true)}
            className='absolute top-2 right-2 font-semibold text-sm flex gap-1 items-center dark:text-zinc-400 dark:hover:text-zinc-300 text-zinc-600 cursor-pointer hover:text-zinc-500 hover:scale-110 transition-all'
          >
            <p>{t('edit')}</p>
            <FaRegEdit />
          </div>
        ...
        {topEight?.length === 0 && !topEightIsLoading && (
          <p className='font-medium italic text-lg text-center text-zinc-500'>
            {t('no top eight')}
          </p>
        )}
        <div className='flex flex-wrap item-center gap-2 items-center'>
