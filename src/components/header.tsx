...
export function Header() {
	...
	return (
		<header
			className={clsx([
				"w-full px-2.5 font-sans sm:px-3 md:px-4 lg:px-5 xl:px-6",
			])}
		>
			<nav className="my-auto flex w-full flex-row justify-between">
				<div
					className={clsx([
						"my-auto flex w-full items-center space-x-3 sm:pr-3",
					])}
				>
					<Link
						href="/"
						className="select-none"
						aria-label="Ethereum Follow Protocol Logo link"
					>
						<Avatar
							...
							className="select-none"
							style={{
								borderRadius: "25%", // Adjust this value for desired roundness
					...
					<Text
						className={clsx([
							"w-22 mx-auto h-20 pt-5 font-bold !leading-4 text-black",
						])}
						trim={"both"}
						weight={"bold"}
						size={{
							...
							<li className="inline font-bold" key={`${index}`}>
								...
								>
									<span
										...
										style={{ color: colorScheme().tertiary }}
									>
										{item.name}
									</span>
								</Link>
