...
type ConnectedAddressFollowing = {
	...
	getConnectedAddressFollowingByAddress: (
		address: Address,
	) => FollowingResponse | undefined;
};
...
export function useConnectedFollowing(): ConnectedAddressFollowing {
	...
	const { data } = useQuery({
		...
		enabled: isConnected,
		queryFn: () =>
			fetchUserFollowing({ addressOrName: connectedAddress as Address }),
	});
...
type ConnectedAddressFollowers = {
	...
	getConnectedAddressFollowerByAddress: (
		address: Address,
	) => FollowerResponse | undefined;
};
...
export function useConnectedFollowers(): ConnectedAddressFollowers {
	...
	const { data } = useQuery({
		...
		enabled: isConnected,
		queryFn: () =>
			fetchUserFollowers({ addressOrName: connectedAddress as Address }),
	});

	return {
		connectedAddressFollowers: data?.followers,
		connectedAddressFollowerAddresses: data?.followers?.map(
...
export function useConnectedProfile(
	...
	const { data } = useQuery({
		...
		queryFn: addressToUse
			? () => fetchUserProfile({ addressOrName: addressToUse })
			: undefined,
	});
	...
	const getFollowingByAddress: EFPProfile["getFollowingByAddress"] = (
		address: Address,
	) => {
		return data.following?.find(
			(follow) => follow.data === address.toLowerCase(),
		);
	};

	const hasFollowingByAddress = (address: Address) =>
		!!getFollowingByAddress(address);

	const getFollowState: EFPProfile["getFollowState"] = (address: Address) => {
		const following: FollowingResponse | undefined =
			getFollowingByAddress(address);
		...
		profile: {
			...
			getFollowerByAddress: (address: Address) => {
				return data.followers?.find(
					(follower) => follower.address === address.toLowerCase(),
				);
			},
