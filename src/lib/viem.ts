...
export const evmClient = {
	"31337": () =>
		createTestClient({
			...
			mode: "anvil",
			transport: http(
				process.env.NEXT_PUBLIC_LOCAL_RPC || "http://0.0.0.0:8545",
			),
		...
		createPublicClient({
			...
			transport: fallback(
				[
					...
					http(
						`https://eth.llamarpc.com/rpc/${process.env.NEXT_PUBLIC_LLAMAFOLIO_ID}`,
					),
					http(
	...
	optimism: () =>
		createPublicClient({
			...
			transport: fallback(
				[
					http(
						`https://rpc.ankr.com/optimism/${process.env.NEXT_PUBLIC_ANKR_ID}`,
					),
					http(
					...
					http(
						`https://optimism.llamarpc.com/rpc/${process.env.NEXT_PUBLIC_LLAMAFOLIO_ID}`,
					),
				],
				{ rank: true },
			),
	...
	sepolia: () =>
		createPublicClient({
			...
			transport: fallback(
				[
					http(
						`https://rpc.ankr.com/eth_sepolia/${process.env.NEXT_PUBLIC_ANKR_ID}`,
					),
					http(
...
export type EVMClient = ReturnType<
	(typeof evmClient)["31337"] | (typeof evmClient)["mainnet"]
>;
