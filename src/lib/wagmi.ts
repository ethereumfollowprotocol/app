...
const config = createConfig({
  ...
  transports: {
    ...
    [optimismSepolia.id]: fallback([
      http(`https://opt-sepolia.g.alchemy.com/v2/${process.env.NEXT_PUBLIC_OP_SEPOLIA_ALCHEMY_ID}`, {
        batch: true,
      }),
      http(
    ...
    [baseSepolia.id]: fallback([
      http(`https://base-sepolia.g.alchemy.com/v2/${process.env.NEXT_PUBLIC_BASE_SEPOLIA_ALCHEMY_ID}`, {
        batch: true,
      }),
      http(`https://smart-cosmological-telescope.base-sepolia.quiknode.pro/${process.env.NEXT_PUBLIC_QUICKNODE_ID}`, {
        batch: true,
      }),
