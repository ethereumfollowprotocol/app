...
export async function follow({
	...
	if (!listClaimCompleted)
		raise("List claim must be completed before following");

	const listRecordToFollow = encodePacked(
		["uint8", "uint8", "address"],
		[1, 1, address],
	);
	const { request } = await client.simulateContract({
		...
		args: [
			slot,
			encodePacked(["uint8", "uint8", "bytes"], [1, 1, listRecordToFollow]),
		],
	});
